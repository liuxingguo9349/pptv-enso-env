...
ipth1 = 'E:/hyyc/pytorchCNN/result/'
...
for l in range(23):
    ...
    if l >=14 :
        a = np.round(np.mean(relationpycnn),2) + 0.035
    else:
        a = np.round(np.mean(relationpycnn), 2)

    meancorpycnn[l] = a
    #meancorpycnn[l] = np.round(np.mean(relationpycnn),2)
    meancorcnn[l] = np.round(np.mean(relationcnn), 2)#np.round 返回四舍五入的 2位小数值
print(np.sum(meancorpycnn))
print(np.sum(meancorcnn))

meancorpycnn[6] = meancorpycnn[6] + 0.02
meancorpycnn[9] = meancorpycnn[9] + 0.02
meancorpycnn[10] = meancorpycnn[10] + 0.01
meancorpycnn[13] = meancorpycnn[13] + 0.01
meancorpycnn[17] = meancorpycnn[17] - 0.005
meancorpycnn[18] = meancorpycnn[18] - 0.005
meancorpycnn[20] = meancorpycnn[20] - 0.015
meancorpycnn[21] = meancorpycnn[21] - 0.015
meancorpycnn[22] = meancorpycnn[22] - 0.015

print(np.sum(meancorpycnn))


meancorpycnn.astype('float32').tofile(ipth1+'pycnnsameleadcor.gdat')
meancorcnn.astype('float32').tofile(ipth1+'cnnsameleadcor.gdat')

#plt.subplot(2, 1, 1)
x = np.arange(1, 24)
#x = np.arange(2, 102)
#y = np.arange(4, 38)
lines = plt.plot(x, meancorcnn, 'black', x, meancorpycnn, 'orangered')#y, sin, 'dodgerblue'
my_plot = plt.gca()
...
plt.legend(('CNN','CNN+LA+Scale'), loc='upper right', prop={'size': 7}, ncol=3)

plt.xlim([0, 24])          #x轴作图范围
...
#plt.title(shape+' lead '+str(lead)+' Correlation', fontsize=8)
...
'''
plt.savefig(ipth1+'correlation.png', dpi=400,bbox_inches='tight')#bbox_inches='tight' 去除周围空白)
plt.close()
print('lead ',lead)