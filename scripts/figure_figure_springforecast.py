...
shape1 = ['gradsameleadmean','samelead','gradmean.gdat','lead1']     #alltransgrad   gradsameleadmean
shape2 = ['alltransgrad','singlemodel','gradmean.gdat','lead1mon12']             #singlemodel    samelead
tmp = shape2
#name = tmp[3]
...
for l in range(10):
    for t in range(3,8):
        name = 'lead'+str(l+1)
        target = str(t+1)
        result = 'bm'+ str(l+1) + 'mon'+ target +tmp[2]

        ipth1 = 'E:/hyyc/pytorchCNN/springforecast/'+tmp[0]+'/'       #数据文件夹
        ipth2 = 'E:/hyyc/pytorchCNN/springforecast/'+tmp[1]+'/'       #保存结果文件夹
        ...
        #heat_each1 = heat_each[j]
        print(heat_each1.max(),heat_each1.min())

        heat_each1 = heat_each1 - np.min(heat_each1)
        heat_each1 = heat_each1 / (np.max(heat_each1))  # 化到0-1之间 因为值太小了，分母不用再加1e-7，会使得最大值不是1
        ...
        space = '                                                                         '
        plt.title('Lead '+str(l+1)+' month '+ target + space + '[El Niño Heatmap]', fontsize=8, y=0.962, x=0.5)
        ...
        plt.savefig(ipth2  + name + 'mon'+ target +'.png', dpi=500,bbox_inches='tight')#默认dpi=100 dpi=155时突然变方块
